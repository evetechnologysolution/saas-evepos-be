...
export const generatePayment = async (data) => {
    try {
        const success = `${data?.baseUrl || process.env.FE_URL}/dashboard/payment/success/${data?._id || 1}`;
        const failed = `${data?.baseUrl || process.env.FE_URL}/dashboard/payment/failed/${data?._id || 1}`;
        const paymentId = data?._id || "ORD-" + (Math.floor(Math.random() * Date.now()) % Math.pow(10, 10));

        let paymentData = {
            external_id: paymentId,
            amount: data.totalPrice,
            description: "Invoice for EvePOS",
            ...
            customer: {
                ...
                mobile_number: data?.customer?.phone ? convertToE164(data?.customer?.phone) : "",
            },
        ...
        const createPaymentResponse = await axios.post(`${XENDIT_URL}/v2/invoices`, paymentData, {
            headers: {
...
export const checkPayment = async (data) => {
    try {
        const response = await axios.get(`${XENDIT_URL}/v2/invoices?external_id=${data?.external_id || data?._id}`, {
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
                Authorization: XENDIT_KEY,
        ...
        // Ambil yang paling baru berdasarkan created
        const latestInvoice = invoices.sort((a, b) => new Date(b.created) - new Date(a.created))[0];

        return {
            status: 200,
            id: latestInvoice?.id || "",
            external_id: latestInvoice?.external_id || "",
            amount: latestInvoice?.amount || "",
            invoice_url: latestInvoice?.invoice_url || "",
            payment_status: latestInvoice?.status || "",
            created: latestInvoice?.created || "",
            updated: latestInvoice?.updated || "",
            currency: latestInvoice?.currency || "",
            payment_channel: latestInvoice?.payment_channel || "",
            payment_destination: latestInvoice?.payment_destination || "",
            payment_id: latestInvoice?.payment_id || "",
        };
    } catch (err) {
        return {
            status: 500,
