...
export const getLogSummary = async (req, res) => {
    try {
        ...
        const progressLabels = await ProgressLabel.find(labelMatch).select("name").sort({ createdAt: 1 }).lean();
        ...
        const merged = initialActivity
            .map((act) => {
                const found = summaryResult.find((r) => r.status?.toLowerCase() === act.status?.toLowerCase());
                return {
                    status: act.status,
                    qty: found?.qty || 0,
                    qtyKg: found?.qtyKg || 0,
...
export const addDataByOrder = async (req, res) => {
    try {
        ...
        const [updatedProgress] = await Promise.all([
            ...
            lockerName ? Order.updateOne(qMatch, { $set: { lockerName } }) : Promise.resolve(null), // skip jika tidak ada lockerName
        ]);
...
export const editData = async (req, res) => {
    try {
        ...
        const updatedData = await Progress.updateOne({ _id: req.params.id }, update);
